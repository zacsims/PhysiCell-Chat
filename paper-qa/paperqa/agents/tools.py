...
async def download_pdf(pmid:str, savename:str) -> None:
    src = FindIt(pmid)
    if src.url is not None:
        logger.info(f"downloading pdf for: {pmid=}.")
        urlretrieve(src.url, savename)
    else:
        logger.info(f'Could not find pdf for {pmid=}')
...
class PaperSearch(NamedTool):
    ...
    retrieval_system: RetrievalSystem = RetrievalSystem("MedCPT", corpus_name="PubMed")
        
    async def retrieve_papers(self, query: str, savepath:str, k:int=10) -> None:
        retrieved_snippets, scores = self.retrieval_system.retrieve(query, k=k, rrf_k=2*k)
        logger.info(f"retrieved {len(retrieved_snippets)} snippets")
        ...
        for snippet in retrieved_snippets:
            ...
            if not os.path.exists(savename): await download_pdf(pmid, savename)
    ...
    async def paper_search(
        ...
        logger.info(f"retrieving papers for query: {query!r}. saving pdfs at {self.settings.agent.index.paper_directory}")
        papers = await self.retrieve_papers(query, self.settings.agent.index.paper_directory)
        ...
        index = await get_directory_index(settings=self.settings, build=True)
